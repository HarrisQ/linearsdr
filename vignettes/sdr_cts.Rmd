...
For demonstrating the linear SDR methods on multivariate responses, we consider the Energy Efficiency dataset available [here](https://archive.ics.uci.edu/ml/datasets/Energy+efficiency).
There are 8 predictors, and from a pairs plot, we can see a number are discrete.
The response is bivariate, with the first response being the heating load, and the second response being the cooling load.

For the inverse SDR methods, we apply the SDR methods component-wise to the response. The first sufficient predictor for each component of the response is taken as a sufficient dimension reduction direction. This is similar to the approach of Cook and Setodji. 
For the forward methods, we consider the entire response simultaneously.
...
For SIR, we use 5 slices and estimate 2 sufficient directions per component. We take the first direction of each component to get two sufficient predictors.  
...
For SAVE, we use 5 slices, the same as SIR, and plot the two estimated sufficient predictors.
...
We use a Gaussian kernel for the local linear weights and standardize the predictors so that the kernel choice is more appropriate. The bandwidth is set to $1.5$ and we consider the first two sufficient predictors against the bivariate response. 
...
b_hat_opg3 = opcg(x=X3_std, y=Y3, bw = 1.5, d=2, ytype = "continuous", 
                 method= "cg", parallelize = T )
...
We use a Gaussian kernel for the local linear weights and standardize the predictors so that the kernel choice is more appropriate. For the starting value of the $\beta$ matrix, we use the default $p \times d$ matrix with upper block as the identity. 
The bandwidth is set to $1.5$ and we consider the first two sufficient predictors against the bivariate response.

![](../man/figures/cts/ex3_mave1.png)

![](../man/figures/cts/ex3_mave2.png)
![](../man/figures/cts/ex3_mave3.png)
...
# 
# start_time1 = Sys.time();
# b_hat_made3 = made(x=(X3_std), Y3, bw=1.5, d=2,
#                   ytype='continuous',
#                   method=list(opcg="cg", made="cg"), B_mat = NULL,
#                   parallelize=T,
#                   control_list=list(print_iter=T, 
...
# end_time1 = Sys.time(); end_time1 - start_time1;
#### For B_mat = NULL
# [1] "MADE: euc_dist dist is" "0.131175506718606"     
# [3] "1"                     
# [1] "MADE: euc_dist dist is" "0.0436618536536756"    
# [3] "2"                     
# [1] "MADE: euc_dist dist is" "0.0152859202750813"    
# [3] "3"                     
# [1] "MADE: euc_dist dist is" "0.00468325544335494"   
# [3] "4"                     
# [1] "MADE: euc_dist dist is" "0.000958786277942029"  
# [3] "5"                     
# [1] "0 - non-convergence"
# > end_time1 = Sys.time(); end_time1 - start_time1;
# Time difference of 18.72442 mins
#
# 
# mave_plot3_1=linearsdr:::ggplot_fsdr(Y3_1, t((X3_std)%*%b_hat_made3[,1]),
#                                      y_on_axis=T,
#                                      ytype="continuous",
#                                      h_lab='MAVE 1', v_lab='Y1',
#                                      main_lab= paste0('MAVE'), size=2.5)
# 
# mave_plot3_2=linearsdr:::ggplot_fsdr(Y3_2, t((X3_std)%*%b_hat_made3[,1]),
#                                      y_on_axis=T,
#                                      ytype="continuous",
#                                      h_lab='MAVE 1', v_lab='Y2',
#                                      main_lab= paste0('MAVE'), size=2.5)
# 
# mave_plot3=linearsdr:::ggplot_fsdr(Y3_1, t((X3_std)%*%b_hat_made3[,1:2]),
#                                    y_on_axis=F,
#                                    ytype="continuous",
#                                    h_lab='MAVE 1', v_lab='MAVE 2',
#                                    main_lab= paste0('MAVE'), size=2.5)
# 
# linearsdr:::ggplot_fsdr(Y3_2, t((X3_std)%*%b_hat_made3[,1]),
#                                    y_on_axis=T,
#                                    ytype="continuous",
#                                    h_lab='MAVE 1', v_lab='MAVE 2',
#                                    main_lab= paste0('MAVE'), size=2.5)
# 
# linearsdr:::save_sdr_plot(mave_plot3_2,filename =
#                             paste0('../man/figures/ex3_mave2.png'),
#                                     width = 900, height = 450, units = "px",
#                                     pointsize = 12,
#                                     bg = "white",  res = 100)
